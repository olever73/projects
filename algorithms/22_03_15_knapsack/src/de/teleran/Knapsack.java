...
public class Knapsack {
    ...
    public static void main(String[] args) {
        ...
        int[][] A;// массив оптимальных стоимостей,1-ый индекс к-во предметов,2-ой размер рюкзака,
                  // значение max  cтоимомть предметов для соотв. рюкзака(набора)
        A = new int[count + 1][];
        for (int i = 0; i < count + 1; i++) {
            A[i] = new int[maxWeight + 1];
        }

        for (int k = 0; k <= count; k++) {  //k  размер набора предметов, s размер рюкзака
